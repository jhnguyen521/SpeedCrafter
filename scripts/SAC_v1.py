...
class SpeedCrafter(gym.Env):
    ...
    def step(self, action):
        ...
        crafted_item = False
        if self.episode_step >= self.max_episode_steps: #TODO: OR GATHERED ALL RESOURCES
            done = True
        ...
        if reward > 0: # We picked up a resource
            self.last_dist = float('inf')
            self.target_pos = None

            for k,v in self.inventory.items():
                if k not in self.last_inventory:
                    self.resources[k] -= 1
                    break
                elif self.last_inventory[k] != v:
                    self.resources[k] -= 1
                    break
